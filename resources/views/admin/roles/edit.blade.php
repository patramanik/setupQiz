...
<div class="mb-6">
    ...
    <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-100">Edit Role: <span class="text-indigo-600 dark:text-indigo-400 capitalize">{{ $role->name }}</span></h2>
</div>
...
<div class="bg-white dark:bg-slate-800/80 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700/50 overflow-hidden">
    <form action="{{ route('roles.update', $role) }}" method="POST" class="p-6 md:p-8">
        ...
        <div class="mb-8">
            <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-4 border-b border-gray-100 dark:border-slate-700 pb-2">Modify Permissions</label>
                    ...
                    <label class="flex items-start gap-3 p-3 rounded-xl border border-gray-100 dark:border-slate-700/50 hover:bg-gray-50 dark:hover:bg-slate-700/30 cursor-pointer transition-colors">
                        <div class="flex items-center h-5 mt-0.5">
                            <input type="checkbox" name="permissions[]" value="{{ $permission->id }}" 
                                @checked(is_array(old('permissions')) ? in_array($permission->id, old('permissions')) : in_array($permission->id, $rolePermissions))
